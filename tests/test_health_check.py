...
class TestNoHitHints:
    ...
    def test_peek_empty_index(self, store):
        ...
        assert "No matching Code Entries" in result or "No projects indexed yet" in result
        assert NO_HIT_HINTS in result
...
class TestCallGraphCleanup:
    def test_call_edges_edges_stored_during_index(self, store, repo_copy):
        store.index_project(str(repo_copy))
    ...
    def test_health_check_removes_orphan_call_edges_edges(
    ...
    def test_absent_project_cleans_up_call_edges(self, store, repo_copy):
        store.index_project(str(repo_copy))
        from hermes_turbomem.project_id import resolve_project
        info = resolve_project(repo_copy)
        edges_before = store._conn.execute(
            "SELECT COUNT(*) FROM call_edges WHERE project_id = ?",
            (info.project_id,),
        ).fetchone()[0]
        assert edges_before > 0
